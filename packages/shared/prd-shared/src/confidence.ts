...
export function assessConfidence(factors: ConfidenceFactors): ConfidenceAssessment {
  ...
  const averageScore =
    totalFactors > 0 ? overallScore / totalFactors : CONFIDENCE_SCORING.DEFAULT_SCORE
...
export function assessContextRichness(contextPayload?: any): ConfidenceLevel {
  ...
  const totalContext =
    contextItems + selectedMessages + (hasCurrentPRD ? CONTENT_THRESHOLDS.PRD_CONTEXT_WEIGHT : 0)
