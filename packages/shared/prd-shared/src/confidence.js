 ...
 */
export function assessInputCompleteness(message, contextPayload) {
 ...
 */
export function assessContextRichness(contextPayload) {
    if (!contextPayload)
        return 'low';
    ...
    if (totalContext >= CONTENT_THRESHOLDS.RICH_CONTEXT_ITEMS)
        return 'high';
    if (totalContext >= CONTENT_THRESHOLDS.ADEQUATE_CONTEXT_ITEMS)
        return 'medium';
    return 'low';
}
/**
 ...
 */
export function assessContentSpecificity(content) {
    if (!content)
        return 'low';
    ...
    if (contentStr.length > CONTENT_THRESHOLDS.LONG_CONTENT_LENGTH)
        specificityScore++;
    if (contentStr.length < CONTENT_THRESHOLDS.SHORT_CONTENT_LENGTH)
        specificityScore--;
    if (specificityScore >= CONTENT_THRESHOLDS.HIGH_SPECIFICITY_SCORE)
        return 'high';
    if (specificityScore >= CONTENT_THRESHOLDS.MEDIUM_SPECIFICITY_SCORE)
        return 'medium';
    return 'low';
}
/**
 ...
 */
export function combineConfidenceAssessments(assessments) {
    ...
    // Combine reasons
    const allReasons = sectionNames.flatMap(name => assessments[name].reasons.map(reason => `${name}: ${reason}`));
    return {
        level: overallLevel,
        reasons: [
            ...
            ...allReasons.slice(0, CONTENT_THRESHOLDS.MAX_COMBINED_REASONS)
        ]
    };
}
