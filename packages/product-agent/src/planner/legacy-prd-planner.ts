...
import type { PlanGraph, RunContext } from '../contracts/core'
...
interface LegacyPrdPlannerOptions {
  clock?: () => Date
}

export class LegacyPrdPlanner implements Planner<PrdPlanTask> {
  private readonly clock: () => Date

  constructor(options?: LegacyPrdPlannerOptions) {
  ...
  async createPlan(context: PrdRunContext): Promise<PlanDraft<PrdPlanTask>> {
    ...
    const sectionNodes: Record<string, ReturnType<LegacyPrdPlanner['createSectionNode']>> = {}
  ...
  async refinePlan(input: PlanRefinementInput<PrdPlanTask>): Promise<PlanDraft<PrdPlanTask>> {
    return {
...
export const createLegacyPrdPlanner = (options?: LegacyPrdPlannerOptions): LegacyPrdPlanner =>
  new LegacyPrdPlanner(options)
