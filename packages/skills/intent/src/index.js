...
export class IntentClassifierSkill {
    ...
    normalizeResult(raw, availableArtifacts) {
        const canonicalArtifacts = new Set(availableArtifacts.map(artifact => artifact.toLowerCase()));
        const normalizeKind = (value) => {
            const lower = value.trim().toLowerCase();
            if (!lower) {
                return 'prd';
            }
            const match = availableArtifacts.find(artifact => artifact.toLowerCase() === lower);
            return match ?? value;
        };
        const chain = [];
