...
const SectionDetectionResultSchema = z.object({
  affectedSections: z.array(
    z.enum(['targetUsers', 'solution', 'keyFeatures', 'successMetrics', 'constraints'])
  ),
...
export class SectionDetectionAnalyzer extends BaseAnalyzer {
  async analyze(
    input: AnalyzerInput & { existingPRD?: unknown }
  ): Promise<AnalyzerResult<SectionDetectionResult>> {
    const result = await this.generateStructured({
      schema: SectionDetectionResultSchema,
      prompt: createSectionDetectionPrompt(
        input.message,
        input.existingPRD || input.context?.existingPRD
      ),
