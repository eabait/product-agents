...
import { createSuccessMetricsSectionPrompt } from '../prompts/index.ts';
import { assessConfidence, assessInputCompleteness, assessContextRichness, assessContentSpecificity, MIN_SUCCESS_METRICS, MAX_SUCCESS_METRICS } from '@product-agents/prd-shared';
const SuccessMetricSchema = z.object({
    ...
    target: z.coerce.string()
        ...
        .refine(value => value.length >= 3, 'Target must be specific'),
    timeline: z.string().min(3, 'Timeline is required')
});
const SuccessMetricOperationSchema = z.object({
    action: z.enum(['add', 'update', 'remove', 'replace']).default('add'),
...
export class SuccessMetricsSectionWriter extends BaseSectionWriter {
    ...
    createSuccessMetricsPrompt(input, contextAnalysis) {
        return createSuccessMetricsSectionPrompt(input, contextAnalysis);
    }
    extractExistingMetrics(existingSection) {
        if (!existingSection)
            return [];
    ...
    validateSuccessMetricsSection(section) {
        const issues = [];
        ...
        const vagueTargets = section.successMetrics.filter(m => !m.target.match(/\d+/) ||
        ...
        const missingTimelines = section.successMetrics.filter(m => !m.timeline ||
            m.timeline.length < 5 ||
...
const sanitizeMetricFields = (metric) => sanitizeMetric({
...
const findMetricIndex = (metrics, name) => {
    if (!name)
        return -1;
    const targetName = name.trim().toLowerCase();
    return metrics.findIndex(m => m.metric.trim().toLowerCase() === targetName);
};
const normalizeSuccessMetricsPlan = (plan) => ({
...
export const applySuccessMetricsPlan = (existingMetrics, plan) => {
    const normalizedPlan = normalizeSuccessMetricsPlan(plan);
    ...
    for (const operation of normalizedPlan.operations ?? []) {
        ...
        const normalizedAction = action === 'replace' ? 'update' : action;
