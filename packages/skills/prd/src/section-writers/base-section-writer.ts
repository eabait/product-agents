...
export abstract class BaseSectionWriter {
  ...
  protected async generateStructuredWithFallback<T>(params: {
    ...
    try {
      ...
      if (
        ...
        console.warn(
          `Model ${this.settings.model} unavailable for section writer ${this.getSectionName()}, falling back to ${fallbackModel}`
        )
   ...
   */
  protected validateSectionContent(content: unknown): { isValid: boolean; issues: string[] } {
    const issues: string[] = []
