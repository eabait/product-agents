...
import { createKeyFeaturesSectionPrompt } from '../prompts/index.ts';
import { assessConfidence, assessInputCompleteness, assessContextRichness, assessContentSpecificity, DEFAULT_TEMPERATURE, MIN_KEY_FEATURES, MAX_KEY_FEATURES, MIN_FEATURE_DESCRIPTION_LENGTH } from '@product-agents/prd-shared';
...
export class KeyFeaturesSectionWriter extends BaseSectionWriter {
    ...
    createKeyFeaturesPrompt(input, contextAnalysis, existingFeatures) {
        return createKeyFeaturesSectionPrompt(input, contextAnalysis, existingFeatures);
    }
    validateKeyFeaturesSection(section) {
        const issues = [];
        ...
        const duplicateFeatures = section.keyFeatures.filter((feature, index, array) => array.findIndex(f => f.toLowerCase().includes(feature.toLowerCase().substring(0, 10))) !== index);
    ...
    extractExistingFeatures(existingSection) {
        if (!existingSection)
            return [];
...
const sanitizeFeatures = (features) => features
    .map(feature => (typeof feature === 'string' ? feature.trim() : ''))
    .filter(feature => feature.length > 0);
const getFeatureKey = (feature) => feature.split(':')[0].trim().toLowerCase();
...
export const applyKeyFeaturesPlan = (existingFeatures, plan) => {
    const normalizedPlan = normalizeKeyFeaturesPlan(plan);
