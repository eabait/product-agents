...
export function createConstraintsSectionPrompt(input, contextAnalysis, existingConstraints, existingAssumptions) {
    const lines = [
    ...
    lines.push('## Instructions', '- Produce 2-6 constraints (non-negotiable limits) and 2-5 assumptions (items to validate).', '- Keep statements specific, actionable, and ≤ 200 characters.', '- When modifying or removing existing items, reference them directly.', '- Incorporate insights from the analysis summary and provided context.', '', formatStructuredOutputRequirement([
