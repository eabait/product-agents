...
export function createTargetUsersSectionPrompt(input, contextAnalysis, existingUsers) {
    const lines = [
    ...
    lines.push('## Instructions', '- Produce 2-4 personas. Each persona should be 1-2 sentences describing who they are and their primary need.', '- Focus on specific user segments that align with the themes and requirements.', '- Avoid generic labels (e.g., "end users"); highlight context such as role, company size, or pain point.', '- Reference existing personas when updating or removing them.', '', formatStructuredOutputRequirement([
